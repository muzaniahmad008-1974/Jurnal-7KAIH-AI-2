...
export const ParentValidationView: React.FC<ParentValidationViewProps> = ({
  ...
  return (
    <div className="space-y-6">
              ...
              return (
                ...
                >
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-3">
                    {/* Tampilan Tanda Tangan / Paraf Jika Sudah Ada */}
                    {journal.parentSignature && (
                      <div className="p-3.5 rounded-2xl bg-white border border-emerald-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-2xs">
                        <div className="flex items-center gap-3">
                          <div className="p-1 bg-blue-50/50 rounded-xl border border-slate-200 shrink-0">
                            <img
                              src={journal.parentSignature}
                          ...
                          <div className="space-y-0.5">
                            <div className="flex items-center gap-2">
                              <span className="text-xs font-bold text-slate-900">
                                {journal.parentValidatorName || currentPersona?.name || 'Orang Tua / Wali'}
                              </span>
                              <span className="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[10px] font-bold">
                                {journal.parentValidationType === 'INITIALS' ? 'Paraf Resmi' : 'Tanda Tangan Digital'}
                              </span>
                            </div>
                            <p className="text-[11px] text-slate-500">
                              {journal.parentValidationSource === 'STUDENT_DASHBOARD'
                                ? '✓ Divalidasi via Dashboard Murid (Pendampingan Langsung)'
                                : '✓ Divalidasi via Dashboard Orang Tua'}
                            </p>
                            {journal.parentValidatedAt && (
                              <p className="text-[10px] text-slate-400">
                                {new Date(journal.parentValidatedAt).toLocaleDateString('id-ID', {
                                  weekday: 'long',
                                  day: 'numeric',
                                  month: 'short',
                                  year: 'numeric',
                                })}{' '}
                                • {new Date(journal.parentValidatedAt).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })} WITA
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            setSelectedJournalToSign(journal);
                            setIsSignatureModalOpen(true);
                          }}
                          className="px-3 py-1.5 rounded-xl border border-emerald-300 text-emerald-800 hover:bg-emerald-50 text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer"
                        >
                          <PenTool className="w-3.5 h-3.5" />
                          <span>Ganti Tanda Tangan / Paraf</span>
                        </button>
                      </div>
                    )}
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
                      <span className="text-[11px] text-slate-500">
                        {validatedSuccessId === journal.id
                          ? '✓ Jurnal berhasil divalidasi dan tersinkronisasi 2 arah!'
                          : `Validasi harian mengonfirmasi ${completedCount} kebiasaan yang dicatatkan ananda.`}
                      </span>
                      <div className="flex items-center gap-2">
                        {/* Tombol Utama: Validasi dengan Tanda Tangan / Paraf */}
                        <button
                          type="button"
                          onClick={() => {
                            setSelectedJournalToSign(journal);
                          ...
                          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#0753A5] hover:bg-blue-700 text-white text-xs font-bold shadow-md transition-all cursor-pointer active:scale-95"
                        >
                          <PenTool className="w-4 h-4 text-emerald-300" />
                          <span>
                            {isFullyValidated
                              ? 'Bubuhi / Perbarui Tanda Tangan'
                              : 'Validasi & Tanda Tangan / Paraf'}
                          </span>
                        </button>
                        ...
                        >
                          <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                          <span>{isFullyValidated ? 'Simpan Catatan' : 'Validasi Cepat'}</span>
                        </button>
                      </div>
                    </div>
                  </div>
      ...
      {/* Modal Tanda Tangan / Paraf Validasi Orang Tua (Dashboard Orang Tua) */}
      {isSignatureModalOpen && selectedJournalToSign && (
        <ParentSignatureModal
          ...
          defaultValidatorName={
            selectedJournalToSign.parentValidatorName || currentPersona?.name || 'Orang Tua / Wali'
          }
          ...
          onSaveValidation={({ journalId, parentName, note, validationType, signatureDataUrl, source }) => {
            ...
            setValidatedSuccessId(journalId);
            showToast('Tanda tangan validasi orang tua berhasil dibubuhkan dan tersinkronisasi!');
            setTimeout(() => setValidatedSuccessId(null), 3000);
          }}
        />
      )}
