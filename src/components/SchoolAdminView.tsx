...
export const SchoolAdminView: React.FC<SchoolAdminViewProps> = ({
  ...
  const defaultSchool = schools[0] || getStoredSchools()[0];
  ...
  const activeSchool = useMemo(() => {
    return (
      ...
      schools.find((s) => isSameSchool(s.name, currentSchoolName)) ||
      defaultSchool
    );
  ...
  const [rombels, setRombels] = useState<Rombel[]>(() => {
    ...
    return all.filter((r) => isRombelOfSchool(r, targetSchoolId, targetSchoolName));
  });
  ...
  const handleSynchronizeMasterData = (notify = true) => {
    ...
    const freshStudents = updatedStudents.filter((s) =>
      isStudentOfSchool(s, targetSchoolId, targetSchoolName)
    );
    const freshRombels = updatedRombels.filter((r) =>
      isRombelOfSchool(r, targetSchoolId, targetSchoolName)
    );
  ...
  useEffect(() => {
    const allS = getStoredStudents();
    const allR = getStoredRombels();
    const scopedS = allS.filter((s) => isStudentOfSchool(s, targetSchoolId, targetSchoolName));
    const scopedR = allR.filter((r) => isRombelOfSchool(r, targetSchoolId, targetSchoolName));
    setStudents(scopedS);
    setRombels(scopedR);
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="space-y-1.5">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="px-2.5 py-0.5 rounded-full bg-blue-100 text-[#0753A5] text-[10px] font-bold">
                    NPSN: {currentSchoolNpsn}
                  </span>
                  <span className="px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[10px] font-bold">
                    Data Rombel & Siswa Tersinkronisasi
                  </span>
                </div>
                {schools.length > 1 && (
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[11px] font-bold text-slate-600">Pilih Satuan Pendidikan:</span>
                    <select
                      value={targetSchoolId}
                      onChange={(e) => setActiveSchoolId(e.target.value)}
                      className="px-2.5 py-1 rounded-lg border border-slate-300 bg-white text-xs font-semibold text-slate-800 cursor-pointer shadow-2xs"
                    >
                      {schools.map((s) => (
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs space-y-5">
            ...
            {dataSubView === 'STUDENTS' && (
              <div className="space-y-4">
                ...
                {/* Banner Sinkronisasi Rombel & Peserta Didik */}
                {studentClassFilter !== 'ALL' && (
                  <div className="p-3.5 rounded-2xl bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 flex flex-wrap items-center justify-between gap-3 text-xs">
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-xl bg-[#0753A5] text-white flex items-center justify-center font-bold text-xs shadow-xs shrink-0">
                        <GraduationCap className="w-4 h-4" />
                      </div>
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-extrabold text-slate-900">
                            Rombongan Belajar: {studentClassFilter}
                          </span>
                          <span className="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 font-bold text-[10px] border border-emerald-300 flex items-center gap-1">
                            <CheckCircle2 className="w-3 h-3 text-emerald-600" />
                            <span>Singkron 100% dengan Tab Rombel ({students.filter((s) => isSameClass(s.className, studentClassFilter)).length} Siswa Terdaftar)</span>
                          </span>
                        </div>
                        <p className="text-slate-600 text-[11px] mt-0.5">
                          Data jumlah siswa terdaftar pada tab Rombel dan tab Data Peserta Didik untuk {studentClassFilter} di {targetSchoolName} telah tersinkronisasi.
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => handleSynchronizeMasterData(true)}
                        className="px-3 py-1.5 rounded-xl bg-white hover:bg-blue-50 text-[#0753A5] font-bold text-xs border border-blue-200 shadow-2xs transition-colors flex items-center gap-1.5 cursor-pointer"
                        title="Sinkronkan Ulang Data Rombel dan Peserta Didik"
                      >
                        <RefreshCw className="w-3.5 h-3.5" />
                        <span>Singkronkan Sekarang</span>
                      </button>
                      <button
                        onClick={() => {
                          setDataSubView('ROMBELS');
                          setRombelSearchQuery(studentClassFilter);
                        }}
                        className="px-3 py-1.5 rounded-xl bg-[#0753A5] hover:bg-blue-700 text-white font-bold text-xs shadow-2xs transition-colors flex items-center gap-1.5 cursor-pointer"
                        title="Lihat di Tab Rombel"
                      >
                        <span>Lihat di Tab Rombel →</span>
                      </button>
                    </div>
                  </div>
                )}
                                ...
                                <div className="flex flex-col gap-1">
                                  <div className="font-bold text-slate-900 flex items-center gap-1.5">
                                    <button
                                      type="button"
                                      onClick={() => {
                                        setDataSubView('STUDENTS');
                                        setStudentClassFilter(r.name);
                                        setStudentStatusFilter('ALL');
                                        setStudentSearchQuery('');
                                      }}
                                      className="group text-left flex items-center gap-1.5 hover:underline cursor-pointer"
                                      title={`Buka Tab Data Peserta Didik untuk melihat ${enrolledCount} siswa ${r.name}`}
                                    >
                                      <span className={enrolledCount > r.capacity ? 'text-amber-600 font-extrabold' : 'text-slate-900 font-extrabold'}>
                                        {enrolledCount} Siswa Terdaftar
                                      </span>
                                      <ExternalLink className="w-3.5 h-3.5 text-[#0753A5] opacity-70 group-hover:opacity-100 transition-opacity" />
                                    </button>
                                  ...
                                  <span className="text-[10px] text-emerald-700 font-bold flex items-center gap-1">
                                    ...
                                    <span>Singkron dengan Data Peserta Didik</span>
                                  </span>
                                </div>
                              </td>
                              ...
                              <td className="py-3 px-4 text-center">
                                <div className="flex items-center justify-center gap-1.5">
                                  <button
                                    ...
                                    className="p-1.5 rounded-lg text-slate-400 hover:text-[#0753A5] hover:bg-blue-50 transition-colors cursor-pointer"
                                    title={`Lihat Data Peserta Didik ${r.name} (${enrolledCount} Siswa)`}
                                  >
